...
echo "DDDDDDDDDDDDDDDDDDDDDDDDDDDDD111111111111111111111111111111111!"

#for num_srvs in `seq 1 25`
for num_srvs in `seq 1 1`
do
    ...
    make -j
    cd ..
    echo "DDDDDDDDDDDDDDDDDDDDDDDDDDDDD222222222222222222222222222222!"

    #for srv_idx in `seq 1 $num_srvs`
    #do
    run_cmd 1 "mkdir -p `pwd`/build/src"
    distribute build/src/main 1
    ...
    #do
    run_cmd 1 "mkdir -p `pwd`/build/bench"
	distribute build/bench/client 1
    ...
    echo "DDDDDDDDDDDDDDDDDDDDDDDDDDDD33333333333333333333333333333!"

    client_pids=
    for clt_idx in `seq $CLT_START_IDX $CLT_END_IDX`
    do
	conf=$DIR/conf/client`expr $clt_idx - $CLT_START_IDX + 1`
	run_program build/bench/client 1 $conf 1>$DIR/logs/$num_srvs.$clt_idx 2>&1 &
	client_pids+=" $!"
    done
    wait $client_pids
